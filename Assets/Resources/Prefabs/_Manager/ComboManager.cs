...
public class ComboManager: ManagedPool<ComboPrefab> {
    ...
    public void Display(Transform transform, int combo) {
        ...
        sequence.Append(prefab.transform.DOLocalMoveY(
            prefab.transform.localPosition.y + CONST.COMBOBOX_FLOATING_DISTANCE,
            CONST.DURATION_COMBO
        ).SetEase(Ease.Linear));
