    ...
    public class PatternHandler
    {
        ...
        public AddBall Append(SlotPrefab slot)
        {
            ...
            if (slot.Generate != null
                || Vector3.Distance(m_selected.First.Value.transform.position, slot.transform.position) > CONST.MAX_DISTANCE)
                return AddBall.none;

            /* is roll-back? */
            if (m_selected.Count > 1
                && slot.Equals(Ray.Instance.Shot(m_selected.First.Next.Value.transform.position)))
            {
            ...
            if (m_selected.Last.Value.Child.BallColor.Equals(slot.Child.BallColor)
                && !m_selected.Contains(slot))
            {
                m_selected.AddFirst(slot);
                return AddBall.add;
            }
