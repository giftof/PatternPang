...
namespace Pattern.Managers
{
    public class SlotManager : IEnumerable
    {
        ...
        private void VerticalSewing(int height, int upIdx, int downIdx, out SlotNode node)
        {
        ...
        private void ZigzagSewing(bool zigzag, SlotNode list1, SlotNode list2, (int way1, int way2) dir1, (int way1, int way2) dir2)
        {
            while (true)
            {
                if (zigzag)
        ...
        private class CustomIEnumerator : IEnumerator
        {
            ...
            private bool MoveUp()
            {
                ...
                if (Board.Link[(int)ClockWise.up].Color != SlotAttribute.generator)
                {
